    ...
    module Admin
      describe CreatePost do
        ...
        context "when everything is ok" do
          ...
          context "and comments authorization handler is enabled" do
            before do
              allow(Rails.application.secrets).to receive(:dig).with(:decidim, :initiatives, :permissions, :comments, :authorization_handler).and_return("true")
            end

            it "creates a new permission" do
              expect do
                subject.call
              end.to change(Decidim::ResourcePermission, :count).by(1)
            end
          end
